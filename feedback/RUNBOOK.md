...
From the repository root, enter this directory and install the
lockfile-pinned Worker toolchain once:

```sh
cd feedback
npm --prefix ../mcp-server/worker ci --ignore-scripts --no-audit --no-fund
```

All Wrangler commands then run through `./wrangler.sh` from this directory.
The wrapper refuses to download an ambient or newer CLI and uses the exact
version pinned in `mcp-server/worker/package-lock.json`.
...
```sh
./wrangler.sh d1 create inference-margins-feedback
```
...
```sh
./wrangler.sh d1 execute inference-margins-feedback --remote --file=schema.sql
```
...
```sh
./wrangler.sh secret put TURNSTILE_SECRET  # paste the Turnstile SECRET key
openssl rand -hex 32                       # generate a salt, then:
./wrangler.sh secret put IP_HASH_SALT      # paste the generated salt
```
...
`site/index.html`). In the public release-snapshot mirror, submit that change
for maintainer review; deployment runs from the private release workflow.
The mirror intentionally does not ship the private `deploy.sh` entrypoint.
...
5. Optional CLI check — metadata columns only, so adversarial bodies never
   ...
   ```sh
   ./wrangler.sh d1 execute inference-margins-feedback --remote \
...
EOF
./wrangler.sh d1 execute inference-margins-feedback --local \
  --persist-to /tmp/im-feedback-dev --file=schema.sql
./wrangler.sh dev --local --port 8787 --persist-to /tmp/im-feedback-dev
