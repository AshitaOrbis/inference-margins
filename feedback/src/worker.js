...
export default {
  async fetch(request, env) {
    ...
    try {
      /* `return await`, not `return`, on every handler below (vetting round 2026-09-19, Astra
         pack C P1-4). Returning a promise from inside `try` resolves it OUTSIDE the try, so a
         rejection — a D1 write that throws, a request body that errors mid-read — escaped this
         catch entirely and `worker.fetch()` itself rejected. The controlled 500, the CORS
         headers and the no-store headers were all bypassed, and what the caller saw was
         whatever the runtime does with a rejected fetch rather than the generic failure this
         handler exists to produce. Reproduced with a request body stream that errors on read:
         `worker.fetch()` rejected with the read error instead of returning 500. */
      ...
      if (path === "/admin" || path.startsWith("/admin/")) {
        ...
        if (path === "/admin/api/review" && request.method === "POST") {
          return await adminMarkReviewed(request, env);
        }
...
async function handleSubmit(request, env) {
  ...
  const ipLimit = await rateLimitAllow(
  ...
  // (9) Reject exact duplicates (whitespace-normalized) that were already stored on a
  // PRIOR day (fast path; the UNIQUE index on body_hash in schema.sql is the permanent
  // backstop). This alone does NOT close the concurrent-submission race — two requests
  // racing the SAME new body can both observe no row here before either has inserted
  // (bq-1015 / inference.feedback_dedupe_quota_race_01); step (10) closes that race.
  ...
  // (10) Global rate limit — atomic Durable Object counter (P1-5), after Turnstile so
  // unverified junk cannot burn the global budget. Idempotency (bodyHash) and quota
  // reservation are made ONE atomic operation here (bq-1015 fix): the global DO tracks
  // which body hashes it has already charged for the active day and increments the
  // counter at most once per hash, closing the race where N concurrent identical
  // requests could each pass the (9) SELECT above (nothing stored yet for any of them)
  // and each separately burn a global slot even though only one row ever gets stored.
  const dayWindow = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
  const globalLimit = intVar(env.RATE_GLOBAL_PER_DAY, 200);
  const globalLimitResult = await rateLimitAllow(
    env, "global", dayWindow, globalLimit, 2 * 86_400_000, bodyHash
  );
  if (globalLimitResult.duplicate) {
    return respond(request, env, 409, "This feedback was already received.");
  }
  if (!globalLimitResult.ok) {
    return respond(
 ...
 */
async function rateLimitAllow(env, name, window, limit, ttlMs, dedupeKey) {
  try {
    ...
    const res = await stub.fetch("https://rate-limiter/increment", {
      ...
      body: JSON.stringify({ window, limit, ttlMs, dedupeKey }),
    });
    if (!res.ok) return { ok: false, duplicate: false };
    const data = await res.json().catch(() => null);
    return {
      ok: Boolean(data && data.ok === true),
      duplicate: Boolean(data && data.duplicate === true),
    };
  } catch {
 ...
 * Stores {window, n, seen}: `seen` is a list of SHA-256 body hashes already
 * charged this window (bq-1015) — a one-way digest, never submission content
 * (quarantine rule) — bounded to at most `limit` entries by the quota itself.
 */
  ...
  async fetch(request) {
    ...
    const ttlMs =
      body && Number.isFinite(body.ttlMs) && body.ttlMs > 0
        ? body.ttlMs
        : 2 * 86_400_000;
    // Optional (bq-1015 fix): when the caller (currently only the "global" feedback
    // counter) passes a dedupeKey, idempotency and quota reservation become ONE atomic
    // operation — see the get→put below and the class comment.
    const dedupeKey =
      body && typeof body.dedupeKey === "string" && body.dedupeKey ? body.dedupeKey : null;
    if (!window || limit <= 0) {
      return Response.json({ ok: false }, { status: 400 });
    }

    let rec = await this.state.storage.get("counter");
    if (!rec || rec.window !== window) rec = { window, n: 0, seen: [] };
    if (!Array.isArray(rec.seen)) rec.seen = []; // records written before this field existed

    // bq-1015 / inference.feedback_dedupe_quota_race_01: a dedupeKey already charged this
    // window is rejected WITHOUT touching the counter, so concurrent requests bearing the
    // same key can never consume more than one slot between them — Workers serializes
    // fetch() calls into the same Durable Object instance (this get→put never interleaves
    // with another call to this same object), so this check-and-reserve is atomic.
    if (dedupeKey && rec.seen.includes(dedupeKey)) {
      return Response.json({ ok: false, duplicate: true });
    }
    if (rec.n >= limit) return Response.json({ ok: false });
    rec.n += 1;
    // seen can never hold more than `limit` entries — one push per accepted increment —
    // so it is bounded by the same quota and needs no separate cap.
    if (dedupeKey) rec.seen.push(dedupeKey);
