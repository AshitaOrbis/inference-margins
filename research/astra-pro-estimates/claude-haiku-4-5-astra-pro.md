# Claude Haiku 4.5 — GPT-6 Astra Pro estimate (2026-09-25)

**Provenance.** One GPT-6 Astra Pro research run (ChatGPT, model `gpt-6-pro`), request `pr-20260925T064303Z-3cdb62`, answered 2026-09-25T08:24:30Z. It was given this calculator's input contract, the Claude Opus 4.x reference estimate and the live connector's scenario space, and asked to set every input for Claude Haiku 4.5 from public evidence. The answer is reproduced below verbatim except for three presentation changes: ChatGPT's interface citation markers are removed, as on every page of this annex; links into the research run's own sandbox (files no reader can open) keep their text and lose the dead target; and equations are set as preformatted text, with a space between a bracket and a parenthesis inside them so the renderer does not read a product as a link; its sources are cited by URL in the text (published text SHA-256 `0ee917add83bfa7ebb690d090c781926e756da244f139f1f9a82a1e67fe6119d`; as received, `0199d7c82168874e8632c70a1e5eb00a246d5879b2f7efc7759a39d6773bb28b`).

## What the site shows

| reading | value |
|---|---|
| **Card headline** — this calculator on the run's central inputs, at list | **~73%** (73.41%) |
| Span — the run's own low- and high-margin scenarios, same engine | 54%–85% (54.36% – 84.54%) |
| The run's own stated reading | 73.4% (54.4–84.5%) |
| Same central inputs at the page's Reference traffic (15:1, 60% cache) — context, not the headline | ~72% |
...
| `cacheWriteShare` | 10 | 10 | 10 |
| `cacheWriteMult` | 125 | 125 | 125 |
| `cacheCost` | 5 | 5 | 5 |
| `batchShare` | 0 | 0 | 0 |
| `discount` | 0 | 0 | 0 |
| `blend` | `{"h200":40,"gb200":20,"tpu7":40}` | `{"h200":40,"gb200":20,"tpu7":40}` | `{"h200":40,"gb200":20,"tpu7":40}` |
| `rentAbsLeg` | `{"h200":3.25,"gb200":5,"tpu7":2.4}` | `{"h200":3.25,"gb200":5,"tpu7":2.4}` | `{"h200":3.25,"gb200":5,"tpu7":2.4}` |
| `util` | 65 | 60 | 75 |
| `stackMult` | 1 | 1 | 1 |
| `trendMonths` | 0 | 0 | 0 |
| `interact` | fast | fast | fast |
