# Claude Fable 5.1 — GPT-6 Astra Pro estimate (2026-09-25)

**Provenance.** One GPT-6 Astra Pro research run (ChatGPT, model `gpt-6-pro`), request `pr-20260925T060752Z-f9b29c`, answered 2026-09-25T07:07:16Z. It was given this calculator's input contract, the Claude Opus 4.x reference estimate and the live connector's scenario space, and asked to set every input for Claude Fable 5.1 from public evidence. The answer is reproduced below verbatim except for three presentation changes: ChatGPT's interface citation markers are removed, as on every page of this annex; links into the research run's own sandbox (files no reader can open) keep their text and lose the dead target; and equations are set as preformatted text, with a space between a bracket and a parenthesis inside them so the renderer does not read a product as a link; its sources are cited by URL in the text (published text SHA-256 `454e2637016ff9cc203a5471a5d3b5ce6efe61d34aa46f33065916f22136e959`; as received, `2efe52ece8c6b833a9f361889d61c14b66f148a538ae679e64b7be5eafeda4e5`).

## What the site shows

| reading | value |
|---|---|
| **Card headline** — this calculator on the run's central inputs, at list | **~84%** (84.34%) |
| Span — the run's own low- and high-margin scenarios, same engine | 68%–90% (68.43% – 90.33%) |
| The run's own stated reading | 84.3% (68.4–90.3%) |
| Same central inputs at the page's Reference traffic (15:1, 60% cache) — context, not the headline | ~89% |
...
| `active` | 400 | 600 | 250 |
| `total` | 8000 | 10000 | 6000 |
| `precision` | fp8 | fp8 | fp8 |
| `priceIn` | 10 | 10 | 10 |
| `priceOut` | 50 | 50 | 50 |
| `cacheReadMult` | 2.5 | 2.5 | 2.5 |
| `billCacheHit` | 92 | 92 | 92 |
| `batchShare` | 0 | 0 | 0 |
| `discount` | 0 | 0 | 0 |
| `blend` | `{"gb200":30,"gb300":20,"tpu7":50}` | `{"gb200":30,"gb300":20,"tpu7":50}` | `{"gb200":30,"gb300":20,"tpu7":50}` |
| `rentAbsLeg` | `{"gb200":5,"gb300":6.5,"tpu7":1.5}` | `{"gb200":5,"gb300":6.5,"tpu7":1.5}` | `{"gb200":5,"gb300":6.5,"tpu7":1.5}` |
| `util` | 65 | 55 | 70 |
| `stackMult` | 1.1 | 1 | 1.1 |
| `trendMonths` | 0 | 0 | 0 |
...
| traffic | 50:1, 92% cache | 50:1, 92% cache | 50:1, 92% cache |
