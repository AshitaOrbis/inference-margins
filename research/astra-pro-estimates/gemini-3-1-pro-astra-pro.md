# Gemini 3.1 Pro — GPT-6 Astra Pro estimate (2026-09-25)

**Provenance.** One GPT-6 Astra Pro research run (ChatGPT, model `gpt-6-pro`), request `pr-20260925T064303Z-45d766`, answered 2026-09-25T08:33:37Z. It was given this calculator's input contract, the Claude Opus 4.x reference estimate and the live connector's scenario space, and asked to set every input for Gemini 3.1 Pro from public evidence. The answer is reproduced below verbatim except for three presentation changes: ChatGPT's interface citation markers are removed, as on every page of this annex; links into the research run's own sandbox (files no reader can open) keep their text and lose the dead target; and equations are set as preformatted text, with a space between a bracket and a parenthesis inside them so the renderer does not read a product as a link; its sources are cited by URL in the text (published text SHA-256 `fdfb41599f322ab827f76c259130d23f7e3df25b89c929d91c3eb7a4d4b4ba6d`; as received, `b85d1203bf4dbb9cb0db6b934b341b9208ea315ac8729b85209bb0d28dc5f0e1`).

## What the site shows

| reading | value |
|---|---|
| **Card headline** — this calculator on the run's central inputs, at list | **~92%** (91.59%) |
| Span — the run's own low- and high-margin scenarios, same engine | 84%–94% (83.85% – 93.86%) |
| The run's own stated reading | 91.6% (83.8–93.9%) |
...
| `rentAbsLeg` | `{"tpu7":1.36}` | `{"tpu7":1.61}` | `{"tpu7":1.36}` |
| `util` | 70 | 65 | 75 |
| `stackMult` | 1 | 1 | 1 |
| `trendMonths` | 0 | 0 | 0 |
| `interact` | balanced | balanced | balanced |
| `hwMode` | rent | rent | rent |
| traffic | 8:1, 40% cache | 8:1, 40% cache | 8:1, 40% cache |
