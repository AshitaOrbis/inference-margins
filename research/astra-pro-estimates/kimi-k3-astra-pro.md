# Kimi K3 — GPT-6 Astra Pro estimate (2026-09-25)

**Provenance.** One GPT-6 Astra Pro research run (ChatGPT, model `gpt-6-pro`), request `pr-20260925T060752Z-9a9c6e`, answered 2026-09-25T06:59:23Z. It was given this calculator's input contract, the Claude Opus 4.x reference estimate and the live connector's scenario space, and asked to set every input for Kimi K3 from public evidence. The answer is reproduced below verbatim except for three presentation changes: ChatGPT's interface citation markers are removed, as on every page of this annex; links into the research run's own sandbox (files no reader can open) keep their text and lose the dead target; and equations are set as preformatted text, with a space between a bracket and a parenthesis inside them so the renderer does not read a product as a link; its sources are cited by URL in the text (published text SHA-256 `7452559cc8201f6a34776cc696e29c0c87370d4fecb596fad7614a3f4e40188b`; as received, `35ce5320136a8b4d32f5fefb49e7f31bae6d0fe55fb89533a33428a4a0213836`).

## What the site shows

| reading | value |
|---|---|
| **Card headline** — this calculator on the run's central inputs, at list | **~88%** (87.78%) |
| Span — the run's own low- and high-margin scenarios, same engine | 77%–92% (76.63% – 91.86%) |
| The run's own stated reading | 87.8% (76.6–91.9%) |
| Same central inputs at the page's Reference traffic (15:1, 60% cache) — context, not the headline | ~85% |
...
| `billCacheHit` | 70 | 65 | 60 |
| `batchShare` | 0 | 0 | 0 |
| `discount` | 0 | 0 | 0 |
| `blend` | `{"h800":100}` | `{"h800":100}` | `{"h800":100}` |
| `rentAbsLeg` | `{"h800":2.2}` | `{"h800":2.6}` | `{"h800":2}` |
| `util` | 65 | 50 | 80 |
| `stackMult` | 1 | 1 | 1 |
| `trendMonths` | 0 | 0 | 0 |
...
| traffic | 12:1, 70% cache | 16:1, 65% cache | 8:1, 60% cache |
