...
> **Provenance class: author-model re-emission, not an independently verified original.** The original pack's full row set lived in conversation-sandbox files that expired. On 2026-07-11 the same GPT-5.6 Pro project was asked to re-emit the pack inline; it reports having located its own retained "22:29 UTC final export" and re-emitting it exactly (run 21m0s, Pro-verified, `completion_path: api`, conversation archived privately). That exactness claim is the model's own and cannot be independently byte-verified — which is why the site's authoritative parameter record is the [adopted grounding ledger](https://margins.ashitaorbis.com/research/grounding-ledger.html), generated from the deployed preset registry. **Where this page and the ledger differ, the ledger wins.**
