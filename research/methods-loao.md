...
> **Addendum (2026-07-19, methodology v2.2 — dated header note, slice-4 docs sweep):** this page
> is a historical record of the 2026-07-10 test and its immediate roofline follow-up below, and
> its own conclusion ("the anchor-fit model stays; the roofline ships as an experimental
> diagnostic only") described the site as it stood THAT DAY. It no longer describes the current
> site. The IM3 program (im3-integration-design.md) later shipped a DIFFERENTLY-DESIGNED roofline
> as the live compute path: per-leg deployed efficiency factor η (not one shared value across all platforms),
> family-transfer for related hardware, and a joint fleet fit only for the platforms with no
> usable per-row anchor — a shape built specifically to answer this page's own identifiability
> objection ("one fitted parameter against one training platform group"). "The calculator
> compresses each accelerator's serving behavior into one scalar" (below) and the anchor-fit
> decision were both true on 2026-07-10 and are NOT true of the live engine today.
...
We commissioned the upgrade path named above: a decode roofline with compute, HBM and fabric terms, 1/batch weight amortization, explicit MTP handling, and **one shared** efficiency factor (η) — the fraction of the active roofline limit actually reached, with no per-platform value — fit on the H800 production anchor alone (η = 31.0%). Full derivation: [the roofline consultation](https://margins.ashitaorbis.com/research/consult-roofline.html); runnable diagnostic: [/tests/roofline-diagnostic.mjs](../tests/roofline-diagnostic.mjs), deployed with the site.
