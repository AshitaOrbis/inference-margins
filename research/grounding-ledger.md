...
| TPU v7 Ironwood | `etaDec` | 0.521 | analyst-set (platform-native aggregate-form bridge, MIXED-BASIS and conservative by choice: two same-platform diagnostics 0.528/0.514, midpoint 0.521, declared band 0.514–0.528 — b9 M1, numerator repaired 2026-09-20 by im-vet-six-repairs; VALID ONLY in this row's declared decodeTrafficBasis); evidence=platform-native-aggregate-bridge; b9 M1 (r4 defect D2, run B §A4/§B1/§C1): the joint fleet fit 0.36142 is REJECTED as evidence for this row — it is a geometric mean over six NVIDIA/Ascend observations containing ZERO TPU data, against the project's own LOAO record of 37% average / 59% worst-case cross-platform transfer error. Replaced by the mean of two SAME-PLATFORM aggregate-form diagnostics. NUMERATOR REPAIR, im-vet-six-repairs 2026-09-20 (vetting finding E2, dive E on the served page): both endpoints now use a DECODE-only numerator, which they did not before. (1) η ≈ 518.86 × 480 GB / (64 × 7.37 TB/s) = 0.528 from the rental anchor — unchanged, because 518.86 was already output-only (“518.86 output tok/s/chip (1K/8K workload)”, tpu-recipes, evidence anchor 16). (2) η ≈ 606 × 400 GB / (64 × 7.37 TB/s) = 0.514 from Google's July 2026 Ironwood playbook. The RETIRED form of this endpoint read η ≈ 677 × 400 GB / (64 × 7.37 TB/s) = 0.574, and 677 is not a decode rate: the playbook reports 677 t/s/chip COMBINED input-plus-output under 1K-in/8K-out, and this project's own published blinded replication says so in terms — “Google reports combined input-plus-output throughput” — then solves the two-workload system for T_o = 606 output t/s/chip. Using the combined figure as a decode numerator overstated this endpoint by 11.7%. The naive alternative 677 × 8/9 = 601.8 (η 0.510) is NOT used: generated end-to-end throughput still carries the prompt-processing burden, which is why the two-workload solve exists. MIXED-BASIS DISCLOSURE, added 2026-09-20 after the Astra xhigh review of this repair: the two endpoints are NOT on one timing convention, and this row no longer claims they are. 518.86 is a benchmark OUTPUT-token rate measured over wall time that still includes prompt processing; 606 is a decode-STAGE rate with that burden solved out. The review is right that rejecting 677 × 8/9 for retaining prompt-processing time, and then keeping 518.86 which retains it too, is inconsistent. Applying the same two-workload decomposition to the rental anchor's own workload rows gives roughly 526.98 decode t/s/chip, a diagnostic of 0.536, and a midpoint of 0.525. That decomposition carries its own assumptions, so 0.525 is not uniquely right either, and this row does not adopt it. It keeps 0.521 for one stated reason: of the two candidate midpoints 0.521 is the LOWER, and a lower η raises modeled cost and lowers every margin this leg feeds — so the mixed basis is resolved in the direction that cannot flatter the page. Treat 0.521 as a conservative analyst assumption with a known basis inconsistency of about 0.004, not as a fully normalized measurement. Midpoint 0.521, declared band 0.514–0.528 (was: midpoint 0.55, band 0.528–0.574). The repair LOWERS this coefficient, so it raises modeled TPU cost and lowers every reading TPU contributes to — recorded here because the direction is the evidence that it is a unit repair and not a tuning. These are weight-only, representation-specific diagnostics (they ignore KV and recurrent-state traffic), so the coefficient is bound to decodeTrafficBasis 'replica-resident-distinct' and is NOT a universal platform efficiency. The retired ≈0.199 value is NOT the alternative: run B §A4 shows it is the coefficient required to reproduce the old anchor INSIDE the malformed per-device identity, absorbing the batch/weight unit mismatch rather than measuring efficiency. |
| TPU v7 Ironwood | `decodeTrafficBasis` | replica-resident-distinct | paired eta representation=replica-resident-distinct; nPhysDeclared=16; sources=research/reviews/im-adv-r4-runB-internal-raw.md §A2/§A4/§B1/§C1; Google Ironwood Qwen 3.5 serving playbook 2026-07-14 (677 tok/s/chip COMBINED input-plus-output under 1K-in/8K-out — Google's own reporting convention; concurrency 64, 4 chips, 400 GB replica footprint). Its DECODE-only equivalent is 606 output t/s/chip, from this project's own two-workload solve in research/gptpro-reports/dive-replication-blinded-2026-07-15.md ("Google proxy": 8000/T_i + 1000/T_o = 9000/3707 and 1000/T_i + 8000/T_o = 9000/677 give T_i = 10,279, T_o = 606), published as site/research/dive-replication-blinded.html; TPU v7 Qwen3-Coder-480B rental anchor (518.86 OUTPUT tok/s/chip, stated concurrency 64 over 4 chips — research/evidence-instances-v22.json anchor 16, verbatim "518.86 output tok/s/chip (1K/8K workload)", https://github.com/AI-Hypercomputer/tpu-recipes/tree/main/inference/ironwood/vLLM/Qwen3-Coder-480B-A35B) |
