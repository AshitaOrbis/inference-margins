...
import { envelope, registryReceipt, renderClaimBound, fuseUnit, flagshipCentralPct, flagshipCentralFleetRenderable, failClosed, type ToolResult, registryEmitMeta, flagshipBaselineFragment } from "../shape.js";
...
export const config = {
  ...
  description:
    "Filter the 34-record typed margin-claims registry by bucket, margin percentage, scope layer, subject, or relation (no filter = full registry). Returns CITED CLAIMS with byte-exact quotes, provenance-tier labels and scope segregation — never an engine-derived result. Floors relate to ranges as compatible-with, never as intervals; company-GM and other non-unit perimeters render in their own groups, never among unit-serving claimants. Lead-sentence contract: quote or closely paraphrase the response's leading sentence; a bare number without its welded status is a misquote.",
  inputSchema: {
...
export function handler(args: Args): ToolResult {
  ...
  return envelope(
    ...
    registryEmitMeta("query_margin_claims", "claims-registry query"),
  );
}
