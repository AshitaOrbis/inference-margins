...
const MODELS = [
  ...
  { id: "gemini", lab: "google", name: "Gemini 3.1 Pro", spec: true, nativeTraffic: "reference",
    ...
    note: "Nobody outside Google knows the sizes — 120B active / 3T total are scenario midpoints (brackets 60–240B / 1–4T). Serves on its own TPUs (blend set to Ironwood; older TPU fleets not modeled) — the dive's derived internal cost ≈ $1.62/chip-hr (registered $5.40 × rentMult 0.30) is the 'dive replay' procurement. List $2/$12 ≤200K. Evidence audit: §10." },
  ...
  { id: "kimi", lab: "moonshot", name: "Kimi K2.7 Code (1T/32B)", spec: false, diveMetric: "output", nativeTraffic: "reference",
    ...
    note: "DISCLOSED: 1T total / 32B active, 384 experts, selective INT4 weights (595GB checkpoint), no MTP head. List $0.95/$4.00, cache reads 20% of input, batch tier at 60% of standard = a 40% discount (NOTE: the page's batchShare control applies a uniform assumed 50% off to every model and does not implement this row's disclosed tariff; the §10 replay runs at 0% batch share, so the headline is unaffected). Production historically on A800/H800 (Mooncake: >100B tokens/day); current fleet undisclosed — blend speculative. NOTE: the activated §10 replay computes $0.593/M output cost and an 85.2% OUTPUT-TOKEN margin; its blended 72.4% headline uses a Mooncake-like mix the dive itself declined to estimate. Evidence audit: §10." },
...
const TIPS = {
  ...
  batchShare: { t: "Batch-API share", b: "Share of traffic on a batch tier priced at a UNIFORM ASSUMED 50% off, applied to every model. This is a page-authored scenario control, not each provider's published batch tariff — Moonshot's disclosed tier, for one, is 60% of standard (a 40% discount), so this control is more generous than that row's real rate. §10 replays run at 0% batch share, so no provider headline depends on it.", s: "" },
...
const DIVE_PROCUREMENT_BASES = Object.freeze({
  ...
  gemini: "owned-strategic-tco",      // derived INTERNAL cost ~$1.62/Ironwood-hr (registered $5.40 x rentMult 0.30) — owned/strategic in meaning, expressed as a rent scalar, not the engine's capex build-up
...
function finalAnswer() {
    ...
    }));
  {
    ...
    fa.higherJustifications = [
      ...
      { groupId: "g5-alderson-90", claims: [hjClaim(ald)],
        ...
        bridge: "the owned-TCO route \u224889.2 at the public-evidence reference is this page's closest construction; 90 remains just beyond every authored route.",
        wouldFlip: "a per-lab basis being published and shown to support ~90 at a matched scope — from this page's owned-TCO construction (\u224889.2 at the public-evidence reference), that requires roughly a further 7.0% cut in cost per billed unit.",
        links: [ald.url], loadOps: hjLoadOps(["alderson-90-unit"]) },
    ];
    fa.decompositionLine = "At the page-adopted 2.5 T size, replacing the declared topology weights {10,15,25,15,20,5,10} with the page-adjudicated evidence-informed NA blend {8,11,19,12,25,8,17} moves the result from \u224858 (58.23) to \u224858 (58.41) — a move of under half a point, upward. The NA blend's two Trainium legs are WITHDRAWN from this default on evidence grounds and their declared weight renormalizes over the remaining five, so this is a declared seven-leg topology set against a five-leg default and the renormalization is inside the move rather than beside it; the serve-feasibility rule removes nothing further at this size. The Legacy 5 T Musk-relative size case (read as likely the prior flagship, Opus 4.6; referent unverified) now computes \u224853, and under it the feasibility rule additionally removes H100.";
  ...
  fa.tokens = {
    planningPoint: pct(fa.planningPoint.marginPct) + " — planning baseline, policy-labeled scenario",
    planningPointLine: "The conservative planning case, priced at low/committed planning rates: " + pct(fa.planningPoint.marginPct)
      + " at the public-evidence reference — policy-labeled scenario (unit serving, not a company gross margin), at the "
    ...
    referenceReadingLine: pct(fa.planningPoint.marginPct)
      + " — planning baseline, policy-labeled scenario. Computed at an algorithmic"
    ...
    c2LabelLine: "Quoted from the r4 adversarial adjudication (run B §C2, 2026-07-25), which is what"
      + " this planning baseline is: \"Transitional public-evidence repair scenario:"
    ...
    bridgeLine: "How the readings relate. The public-evidence reference sits numerically below the"
      ...
      + " 2026-07-26 raised it to the " + pct(fa.planningPoint.marginPct) + " planning baseline above. The"
    ...
    basisDeclarationLine: "Every calculator figure in the explanations below is the planning"
      + " baseline — an algorithmic lead of 0 months, family multipliers at 1.0× — unless that"
...
const MARGIN_CLAIMS = [
  ...
  { id: "openai-70-compute-cohort", who: "OpenAI (internal figure)",
    ...
    numeric: { lo: 70, hi: 70 },
    subjectScope: "OpenAI paid-user compute margin (NOT API token margin, NOT a company gross margin), Oct 2025",
