...
const RUBIN = { name: "Vera Rubin NVL72 (proj.)", flopsFp8: 17.5, fp4: true, hbm: 288, bw: 22.0, tdp: 1.80, rent: 8.50, capex: 120000, effDec: 0.13, effPre: 0.45, note: "Preliminary NVIDIA specs (verified Jul 2026): 17.5 PF dense FP8/FP6 per GPU, 288GB HBM4 @ 22 TB/s; the headline ~50 PF figure is SPARSE NVFP4 inference. No serving anchor exists — MFU, rent and capex are projections. 2026-07-15 dive confirms: no MLPerf submission, no InferenceX result (listed 'Coming Soon'), no public rental/purchase price for Rubin or Rubin Ultra — absolute economics remain fully unanchored. NVIDIA's only public claim is RELATIVE (Kimi-K2-Thinking, 32K-in/8K-out): up to 10× tok/s/MW and ~1/10 cost per M tokens vs GB200 NVL72 — not an absolute anchor. Current rack-scale product is Vera Rubin NVL72; do not collapse NVL144/Rubin CPX/Rubin NVL8/R100/VR200/Rubin Ultra into one generic 'Rubin' figure." };
...
const TIPS = {
  ...
  precision: { t: "Serving precision", b: "FP8 is the widely-assumed frontier serving norm (closed labs don't disclose serving precision; DeepSeek's disclosed stack runs FP8); FP4 (Blackwell+) gives ~1.85× throughput where quality holds. This 1.85× is a WHOLE-MODEL FP4 system benchmark (InferenceX GB300): models that quantize only selected expert weights while keeping attention, KV, routing and shared components at higher precision will not realize the full factor, and part of the gain may already be absorbed into the fitted effective-MFU — treat it as an upper-bound scalar, not a guaranteed multiplier. bf16 halves throughput vs FP8.", s: "xjdr: GLM 5.2 fp8 experts+KV ≈ zero quality loss; NVFP4 slight regression. InferenceX: GB300 FP4 = 32× H100, FP8 = 17×." },
