...
const PAIRS = [
  ...
  /* b9 M6 (FA memo §6.1 D-6g): `.explain-body` — the explanation typography class — over BOTH
     backgrounds it can sit on, because the "which background" error is exactly what produced the
     R-1 P0-3 finding (contrast computed against --tile when the FA card is .tile -> --surface-1).
     Registering both stacks means that error cannot recur silently in the implementation. */
  ["explain body (FA card)",          "ink-2",      ["page", "surface-1"]],
  ["explain body (popup, page-like)", "ink-2",      ["page"]],
  ["explain segment label (card)",    "ink-1",      ["page", "surface-1"]],
  ["explain segment label (popup)",   "ink-1",      ["page"]],
  ["explain trigger (card)",          "accent-ink", ["page", "surface-1"]],
  ["explain trigger (popup)",         "accent-ink", ["page"]],
  ["low-evidence affordance text",    "ink-2",      ["page", "surface-1"]],
];
