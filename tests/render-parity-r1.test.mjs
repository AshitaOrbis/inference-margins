...
// Run: node tests/render-parity-r1.test.mjs
...
/* ============================================================================================
   ...
   ============================================================================================ */
const PRE_T3_STRESS_270_SHA256 = "b21afb37a2ac5dabcafecf61ada9932289aa29adad282baa66ea1b6644302dc6";  // re-minted 2026-09-20 by im-vet-six-repairs (program bq-2835, vetting findings E1/E2/E3/E4/E5 + the N1 vocabulary release edit); see the declared-delta block at the head of this file
...
const PRE_T3_STRESS_273_SHA256 = "01f160516c07dcc621bfe75e2573d3c462d0c618b83f5c6f0befd29428b0dd8d";  // re-minted 2026-09-20 by im-vet-six-repairs (program bq-2835, vetting findings E1/E2/E3/E4/E5 + the N1 vocabulary release edit); see the declared-delta block at the head of this file
const HISTORICAL_270_SHA256 = "c3e001c99110db4a1e9e7a5cf7bd5e5521892a845a7b6eb7c6c5749e5eb983fa";  // re-minted 2026-09-20 by im-vet-six-repairs (program bq-2835, vetting findings E1/E2/E3/E4/E5 + the N1 vocabulary release edit); see the declared-delta block at the head of this file
const BASELINE_SHA256 = "4d4124594ebbf2768797a836bdfc2793233848318a5be971594090fc6bb59331";  // 298 states;  // re-minted 2026-09-20 by im-vet-six-repairs (program bq-2835, vetting findings E1/E2/E3/E4/E5 + the N1 vocabulary release edit); see the declared-delta block at the head of this file
const PRE_T2_ELECTRICITY_SHA256 = "2e0cf7ce55c4fd97297238c44aeb3086cc863494075beaa44fb1b2e9e9afa6d7";  // re-minted 2026-09-20 by im-vet-six-repairs (program bq-2835, vetting findings E1/E2/E3/E4/E5 + the N1 vocabulary release edit); see the declared-delta block at the head of this file
...
const expectedStressMovement = [
  { model: "opus", perspective: "stress-public-rate", projectedMarginPct: 58.43046405779231,
    liveMarginPct: 58.43192413475554, deltaPp: 0.0014600769632333588 },
  { model: "sonnet", perspective: "stress-public-rate", projectedMarginPct: 53.858759972560954,
    liveMarginPct: 53.86347352358034, deltaPp: 0.004713551019386308 },
  { model: "haiku", perspective: "stress-public-rate", projectedMarginPct: 59.40150950256131,
    liveMarginPct: 59.404987529877396, deltaPp: 0.0034780273160839315 },
];
...
{ const { readFileSync, readdirSync, statSync } = await import("node:fs");
  ...
  assert("R1 freeze: site/index.html byte-identical apart from its validated release stamp",
    ...
    /* RE-MINTED 2026-09-10 (im-release-edit-r3) FOR ONE CLOSING TAG — bq-2195, and it is a repair,
       ...
       every number written in is the number the engine executes TODAY, measured, not copied.
         ...
         * the page subtitle: "about 63% with the page's adopted 3-month algorithmic lead" -> "about
           ...
           today at 68.41398315513409. Left uncorrected it also made 63 mean two different quantities
    ...
    indexHash === "4f2384d8da1258b1f134792f4336f561ecea6774051532142023033417661f4f"
  ...
  assert("source freeze: site/engine.js matches the reviewed whole-file hash", fh("site/engine.js") === "537460c463b8b8ca2a8726c12bcaa565fe659a05632c9e5aa722427159b24338" /* re-minted 2026-09-20 by im-repo-replacement, owner ruling d-20260920-im-replace-repo-then-publish-deploy, after an Astra xhigh review found the authorised prose correction had landed on the PAGE and not in the calculator's own reader copy: TIPS.energy still told a reader that idle accelerators draw no board power (the very sentence this release corrects, and one this page's own Google annex refutes), the implementation comment repeated it, and BOTH copies of TIPS.procBasis/TIPS.hwMode still called a mixed rented/owned fleet a suite-enforced error while the page now states that it is allowed and declared. Eleven lines, all of them TEXT. PROVED text-only rather than asserted: every one of the 288 model x perspective pairs was computed on this engine and on the committed one from matched complete checkouts of site/, and all 288 agree to twelve decimal places with no error mismatches — which is why every WIDE state hash in this file is unchanged and was re-executed. */ /* RE-MINTED AGAIN 2026-09-20 by im-vet-six-repairs, ASTRA xhigh FOLD: the withdrawal is now disclosed on the exploration route cards and in the coverage sentence (findings 1 and 2), E2 is relabelled MIXED-BASIS with the inconsistency and the 0.525 alternative stated (finding 3), E3 names its decode evidence per leg (finding 5), the r4 §C2 quotation is RESTORED after the N1 pass had changed a word inside it, four reader strings the old checker scope had hidden are renamed including the hero label, and the round-3 card states the rounding convention it actually uses (finding 8). */   /* re-minted 2026-09-20 by im-vet-six-repairs: the Trainium withdrawal's membership path + exclusion clause (E1), three evidence-label downgrades (E5), the two GB-class precision-identity corrections (E2), the estimate-card and final-answer copy rewritten to the vector that runs (E4), the corrected 72.45% input share with its annex link (E3), and the N1 vocabulary release edit */, fh("site/engine.js"));
  ...
  assert("R1 freeze: all MCP TypeScript sources + Worker-native emitters byte-identical (concatenated hash)",
    ...
    mhHex === "2e3efd93b2e3c2a4147a56df2e9492ed2849acb5d7ae95ac1a433eb7ef4bfb91" /* RE-MINTED AGAIN 2026-09-20 by im-vet-six-repairs, ASTRA xhigh FOLD: the withdrawal is now disclosed on the exploration route cards and in the coverage sentence (findings 1 and 2), E2 is relabelled MIXED-BASIS with the inconsistency and the 0.525 alternative stated (finding 3), E3 names its decode evidence per leg (finding 5), the r4 §C2 quotation is RESTORED after the N1 pass had changed a word inside it, four reader strings the old checker scope had hidden are renamed including the hero label, and the round-3 card states the rounding convention it actually uses (finding 8). */   /* re-minted 2026-09-20 by im-vet-six-repairs: the N1 vocabulary release edit renames the connector's "cost lens" strings to "scenario preset" in labels.ts, tools/list_scenario_space.ts, tools/run_scenario.ts and dcmap/economics/legacy.ts, and the Worker build's two engine-ground-truth pins move with the E1/E2 repairs. NO behaviour changes; the transport-text deltas are declared in mcp-server/test/report-text-parity.test.mjs. */  /* re-minted 2026-09-19: the lens-scenario epistemic status, its label and its must_carry rider in labels.ts / shape.ts / tools/run_scenario.ts */, mhHex); }
